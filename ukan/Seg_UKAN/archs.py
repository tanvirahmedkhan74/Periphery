...
class UKAN(nn.Module):
    ...
    drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm, depths=[1, 1, 1], **kwargs):
        ...
        # Normalization layers for tokenized features
        self.norm3 = norm_layer(embed_dims[1])
        self.norm4 = norm_layer(embed_dims[2])

        # Tokenized KAN Blocks
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]
        ...
        # Decoder layers (if needed for feature refinement)
        self.decoder1 = D_ConvLayer(embed_dims[2], embed_dims[1])
        self.decoder2 = D_ConvLayer(embed_dims[1], embed_dims[0])
        self.decoder3 = D_ConvLayer(embed_dims[0], embed_dims[0]//4)
        self.decoder4 = D_ConvLayer(embed_dims[0]//4, embed_dims[0]//8)
    ...
    def forward(self, x):
        ...
        # Encoder stages (feature extraction)
        out = F.relu(F.max_pool2d(self.encoder1(x), 2, 2))
        t1 = out
        out = F.relu(F.max_pool2d(self.encoder2(out), 2, 2))
        t2 = out
        ...
        # Decoder (optional for feature refinement)
        out = F.relu(F.interpolate(self.decoder1(out), scale_factor=(2, 2), mode='bilinear'))
        out = torch.add(out, t4)
        ...
        # Return feature maps (you can choose what layers to return here)
        return out  # Or return other intermediate outputs if needed
