...
def parse_args():
    ...
    parser.add_argument('--lc', default=None, type=str,
                        help='Path to the checkpoint to load before training')
...
def get_dataset_config(dataset_name, config):
    """
    Given a dataset name, return the corresponding dataset loading configuration.

    Args:
        dataset_name (str): The name of the dataset to load.
        config (dict): A dictionary containing dataset configurations with keys 'name', 'image_dir', 'image_ext', 'mask_dir', and 'mask_ext'.

    Returns:
        dict: Configuration with image and mask directories and extensions for the specified dataset.

    Raises:
        ValueError: If the dataset name is not found in the configuration.
    """

    if dataset_name not in config['name']:
        raise ValueError(f"Dataset '{dataset_name}' not found in the configuration.")

    # Get the index of the dataset name
    index = config['name'].index(dataset_name)

    # Return the corresponding configuration
    dataset_config = {
        'image_dir': config['image_dir'][index],
        'image_ext': config['image_ext'][index],
        'mask_dir': config['mask_dir'][index],
        'mask_ext': config['mask_ext'][index]
    }
...
def main():
    ...
    # Load checkpoint if `--lc` argument is provided
    if config['lc'] is not None:
        print(f"Loading checkpoint from {config['lc']}...")
        checkpoint = torch.load(config['lc'])

        # Check if the checkpoint contains a 'model' key, else directly load state_dict
    ...
    dataset_configurations = {
        'name': ['DUTS-TR', 'ECSSD', 'DUT-OMRON'],
        'image_dir': ['DUTS-TR-Image', 'images', 'DUT-OMRON-image'],
        'image_ext': ['.jpg', '.jpg', '.jpg'],
        'mask_dir': ['DUTS-TR-Mask', 'ground_truth_mask', 'pixelwiseGT-new-PNG'],
        'mask_ext': ['.png', '.png', '.png']
    }

    data_config = get_dataset_config(config['dataset'], dataset_configurations)
    # Data loading code
    img_ids = sorted(glob(os.path.join(config['data_dir'], config['dataset'], data_config['image_dir'], '*' + data_config['image_ext'])))
    ...
    train_dataset = Dataset(
        ...
        img_dir=os.path.join(config['data_dir'], config['dataset'], data_config['image_dir']),
        mask_dir=os.path.join(config['data_dir'], config['dataset'], data_config['mask_dir']),
        img_ext=data_config['image_ext'],
        mask_ext=data_config['mask_ext'],
